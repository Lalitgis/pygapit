...
from pygapit import (
    GAPIT,
    read_numeric, read_phenotype, align_taxa, maf_filter,
    vanraden_kinship, compute_pca, build_covariate_matrix,
    emma_remle, genomic_inflation_factor, bonferroni_threshold,
    glm_gwas, mlm_gwas, blink_gwas, farmcpu_gwas,
    gblup, sblup,
    manhattan_plot, qq_plot, kinship_heatmap, pca_plot_2d, gs_scatter,
)

# ── Paths ────────────────────────────────────────────────────────────────────
BASE = os.environ.get('GAPIT_DATA', os.path.join(os.path.dirname(__file__), '..', '..', 'GAPIT', 'Documents'))
OUT  = "./pygapit_demo_output"
os.makedirs(OUT, exist_ok=True)

PHENO_FILE = os.path.join(BASE, "mdp_traits.txt")
GD_FILE    = os.path.join(BASE, "mdp_numeric.txt")
GM_FILE    = os.path.join(BASE, "mdp_SNP_information.txt")

# ── Check data availability ───────────────────────────────────────────────────
for fp in [PHENO_FILE, GD_FILE, GM_FILE]:
    if not os.path.exists(fp):
        print(f"ERROR: Demo data not found at {fp}")
        print("Clone GAPIT repo first: git clone https://github.com/jiabowang/GAPIT.git")
        sys.exit(1)
...
print(f"Phenotype:  {Y.shape[0]} individuals, {Y.shape[1]-1} traits: {Y.columns[1:].tolist()}")
print(f"Genotype:   {GD.shape[0]} individuals, {GD.shape[1]-1} SNPs")
print(f"Map:        {GM.shape[0]} SNPs across chromosomes {sorted(GM['Chromosome'].unique())}")
print(f"Missing phenotype: EarHT={Y['EarHT'].isna().sum()}, "
...
for key, r in results.items():
    ...
    print(f"  {r.model:10s}  h²={r.h2:.3f}  λ={r.lambda_gc:.3f}  "
...
print(f"Working set: n={len(y)} individuals, m={GD_arr.shape[1]} SNPs (after MAF≥0.05 filter)")
...
print(f"  Variance explained: PC1={pca.var_explained[0]:.1%}, "
...
t0 = time.time()
farm_r = farmcpu_gwas(y, X0, GD_arr, chromosomes=chromosomes,
                       positions=positions, max_iterations=10)
...
print(f"λ={lam:.3f}, {len(farm_r.selected_qtns)} QTNs, {sig} sig [{time.time()-t0:.1f}s]")
...
print(f"λ={lam:.3f}, {len(blink_r.selected_qtns)} QTNs, {sig} sig [{time.time()-t0:.1f}s]")
...
for i in top5:
    print(f"  {taxa[i]:12s}  GEBV={gs.gebv[i]:+7.2f}  PEV={gs.pev[i]:.1f}  "
...
for ax, (model_name, pvals) in zip(axes, gwas_results.items()):
    ...
    for c in chroms:
        if c not in seen:
            unique_chr.append(c)
            seen.add(c)
    offset, x_vals, chr_centers = {}, np.zeros(len(pvals)), {}
    cum = 0
    ...
    colors = ["#3C5587" if i % 2 == 0 else "#89A8D0"
              for i, ch in enumerate(unique_chr) for _ in np.where(chroms == ch)[0]]
    ...
    ax.set_title(f"{model_name}  (λ={genomic_inflation_factor(pvals):.3f})",
                 fontsize=10, fontweight="bold")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
plt.tight_layout()
plt.savefig(f"{OUT}/demo_manhattan_all_models.pdf", dpi=120, bbox_inches="tight")
plt.close()
...
for ax, (model_name, pvals) in zip(axes, gwas_results.items()):
    ...
    ax.scatter(np.sort(expected)[::-1], observed,
               c="#3C5587", s=3, alpha=0.6, linewidths=0)
    ...
    ax.spines["right"].set_visible(False)
plt.tight_layout()
plt.savefig(f"{OUT}/demo_qq_all_models.pdf", dpi=120, bbox_inches="tight")
plt.close()
print(f"  Saved: {OUT}/demo_qq_all_models.pdf")

# Kinship heatmap
fig = kinship_heatmap(K[:50, :50], taxa=taxa[:50], title="Kinship (first 50 lines)",
                       save_path=f"{OUT}/demo_kinship.pdf")
plt.close(fig)
print(f"  Saved: {OUT}/demo_kinship.pdf")

# PCA
fig = pca_plot_2d(pca.scores, pca.var_explained, title="PCA — Maize inbred lines",
...
fig = gs_scatter(y, gs.prediction, trait_name="EarHT",
                  save_path=f"{OUT}/demo_gs_scatter.pdf")
plt.close(fig)
...
print(f"  True QTN positions (chr:pos): ", end="")
...
print(f"QTN overlap: {overlap_blink}/{min(total_selected, 20)} "
...
print("  Demo complete!")
print("=" * 65)
print(f"\nOutput files written to: {os.path.abspath(OUT)}/")
print("\nKey results (EarHT trait):")
print(f"  Heritability (h²):     {remle.h2:.4f}")
print(f"  Genomic inflation (λ): {genomic_inflation_factor(glm_r.p_values):.3f} [GLM]  "
